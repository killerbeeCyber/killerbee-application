...
export default function MenuRouter(props: IProps) {
  ...
  useEffect(() => {
    ...
    }else if (location.pathname == '/createProces') {
  ...
  return (
    <div className="menuRouter">
      ...
      {!isLogged &&
        ...
        {isLogged &&
        <>
        <Link to="/createProcede">
          <button
              className={openedPage == 'Créer Procedes' ? 'menuButton menuButton__clicked' : 'menuButton'}
              onClick={() => {
                setSelectedButton(setOpenedPage)
              }}
            >
